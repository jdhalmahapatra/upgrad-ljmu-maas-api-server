...
module.exports = function () {
    ...
    app.use(session({
        secret: maas.config.get('session_secret'),
        cookie: {maxAge: 3600000}, // 1 hour
        unset: 'destroy',
        store: new mongoStore({
            db: maas.mongoose.connection.db,
    ...
    app.use(morgan(':remote-addr - :remote-user ":method :url" :status ":message"', { "stream": maas.logger.stream }));
    // debug logger
