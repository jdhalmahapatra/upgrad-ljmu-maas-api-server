    ...
    $.ajax({
        ...
        success: function (data, status, jqXHR) {
            ...
            if ($("#remember").prop("checked")) {
                $.cookie("maasRemember", credentials.username, {path: '/'}); // cookie valid across entire site
            } else {
                $.removeCookie("maasRemember");
            }

            // store a session cookie
            var expires = (data.sessionInfo.maxLength).seconds().fromNow();
            $.cookie("maasData", data, {expires: expires, path: '/'}); // expiring cookie, valid across entire site
